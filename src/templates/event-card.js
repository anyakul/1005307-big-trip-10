...
const createCardTemplate = ({type, basePrice, destination, dateFrom, dateTo}) => {
  ...
  return (
    ...
    <div class="event__schedule">
      <p class="event__time">
        <time class="event__start-time" datetime="${fullDateFrom}">${castDateFrom}</time>
        &mdash;
        <time class="event__end-time" datetime="${fullDateTo}">${castDateTo}</time>
      </p>
...
const createEventCardTemplate = (events) => {
  const extraServicesMarkup = createExtraServicesMarkup(events.offers);
  ...
  return (
    `<li class="trip-events__item">
      <div class="event">
        ...
        <ul class="event__selected-offers">${extraServicesMarkup}</ul>
